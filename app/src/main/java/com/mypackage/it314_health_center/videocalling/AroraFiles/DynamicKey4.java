...
public class DynamicKey4 {
   ...
   * @param channelName name of channel to join, limited to 64 bytes and should be printable ASCII
   *     characters
  ...
  public static String generateRecordingKey(
      ...
      throws Exception {
    return doGenerate(
        appID, appCertificate, channelName, unixTs, randomInt, uid, expiredTs, RECORDING_SERVICE);
  }
   ...
   * @param randomInt salt for generating dynamic key
   * @param uid user id, range from 0 - max uint32
   * @param expiredTs service expiring timestamp. After this timestamp, user will not be able to
   *     stay in the channel.
   * @return String representation of dynamic key
   * @throws Exception if any error occurs
   */
  ...
  private static String doGenerate(
    ...
    String randomIntStr =
        ("00000000" + Integer.toHexString(randomInt))
    ...
    String expiredTsStr =
        ("0000000000" + Integer.toString(expiredTs))
    ...
    return String.format(
        "%s%s%s%s%s%s", version, signature, appID, unixTsStr, randomIntStr, expiredTsStr);
  }
