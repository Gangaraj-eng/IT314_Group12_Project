...
public class DynamicKey5 {
  ...
  public static String generateDynamicKey(
      ...
      throws Exception {
    String signature =
        generateSignature(appCertificate, service, appID, ts, salt, channel, uid, expiredTs, extra);
    DynamicKey5Content content =
        new DynamicKey5Content(
            service, signature, new Hex().decode(appID.getBytes()), ts, salt, expiredTs, extra);
  ...
  public static String generateInChannelPermissionKey(
    ...
    return generateDynamicKey(
        appID, appCertificate, channel, ts, salt, uid, expiredTs, extra, IN_CHANNEL_PERMISSION);
  }
